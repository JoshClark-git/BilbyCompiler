...
public class TypeRangeADDCodeGenerator implements SimpleCodeGenerator {
	
	
	public TypeRangeADDCodeGenerator() {
		super();
	}
	...
	public ASMCodeFragment generate(ParseNode node, List<ASMCodeFragment> args) {
		...
		int additiveSize = node.child(0).getType().atomicType().getSize();
		
		int subTypeSize = node.child(1).child(0).getType().atomicType().getSize();
		
		if(subTypeSize < node.child(1).child(1).getType().atomicType().getSize()) {
			subTypeSize = node.child(1).child(1).getType().atomicType().getSize();
		}
